...
"""
    Usage example:
        python ARUNet-3D-PR-RandSplit-training.py --gpuid 0 --dataseed 3
...
def main():
    ...
    parser.add_argument("--dataseed", type=int, default=0, help="Random seed for data splits")
    args = vars(parser.parse_args())
    
    device_num = args["gpuid"]
    rnd_seed = args["dataseed"]
    print("GPU ID: " + str(device_num))
    print("Datasplit seed: " + str(rnd_seed))
    num_devices = 1
    ...
    model_filename_base = "./results/BAMC_PNB-CLEANED_ARUNet-3D-RandSplit-Mean_StdDevOverTime_RawFrame-yuriArteryData-32s-Seed" + str(rnd_seed)
    ...
    print(str(data_partition[0]))
    print(str(data_partition[1]))
    print(str(data_partition[2]))

    f = open("data_splits_seed" + str(rnd_seed)+ ".txt","w")
    f.write("Train: " + str(data_partition[0]) + " \n")
    f.write("Val: " + str(data_partition[1]) + " \n")
    f.write("Test: " + str(data_partition[2]) + " \n")
    f.close()
    ...
    train_files.append(
            [
                {"image": img, "label": seg}
                for img, seg in zip(
                    [im for im in all_images if any(pref in im for pref in data_partition[0])],
                    [se for se in all_labels if any(pref in se for pref in data_partition[0])])
            ]
        )
    val_files.append(
            [
                {"image": img, "label": seg}
                for img, seg in zip(
                    [im for im in all_images if any(pref in im for pref in data_partition[1])],
                    [se for se in all_labels if any(pref in se for pref in data_partition[1])])
            ]
        )
    test_files.append(
            [
                {"image": img, "label": seg}
                for img, seg in zip(
                    [im for im in all_images if any(pref in im for pref in data_partition[2])],
                    [se for se in all_labels if any(pref in se for pref in data_partition[2])])
            ]
        )
    ...
    train_ds = CacheDataset(data=train_files[0], transform=train_transforms,cache_rate=1.0, num_workers=num_workers_tr)
    train_loader = DataLoader(train_ds, batch_size=batch_size_tr, shuffle=True, num_workers=num_workers_tr)
    val_ds = CacheDataset(data=val_files[0], transform=val_transforms, cache_rate=1.0, num_workers=num_workers_vl)
